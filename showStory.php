...
<?php
	...
	if (!empty($_POST['storyDate']) ) { // If user has input a date, stories that submitted in this date would be selected from database and show them in the screen
	    $selectedDay=$_POST['storyDate'];
	    $startTime=$selectedDay." 00:00:00";
	    $endTime=$selectedDay." 23:59:59";

	    $stmt = $mysqli->prepare("select story_id, story_title, story_time from story where story_time between ? and ? order by story_time desc");
	    if(!$stmt){
		...
		while($stmt->fetch()){ // Show all stories one by one
			printf("\t<li><p><a href='readStory.php?story_id=%d'>%s.%s</a></p></li>\n",
